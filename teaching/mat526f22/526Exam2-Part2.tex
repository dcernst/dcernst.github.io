...
\]
%NEXT TIME PROVIDE HINT.  MAYBE THIS:
%\emph{Hint:} The general strategy is to mimic and refine the argument we utilized for counting Dyck paths.  In that argument, for non-Dyck paths we reflected ``tail" of path after the first point below the the line $y$-axis. For this argument, identify the first minimum valley and consider changing the E step just prior into a N step and keeping everything else the same.
