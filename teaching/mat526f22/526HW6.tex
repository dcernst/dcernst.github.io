...
\item The number of \emph{maximal increasing runs} of a permutation, denoted $\runs(w)$, where a maximal increasing run is a substring $w(i)<w(i+1)<\cdots < wIi+r)$ such that $w(i-1)$ is not smaller than $w(i)$ and $w(i+r)$ is not smaller than $w(i+r+1)$.
